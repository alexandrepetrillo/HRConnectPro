-- V004: Création de la table DLQ (Dead Letter Queue) pour stocker les messages Kafka en erreur
...
COMMENT ON TABLE dlq_messages IS 'Dead Letter Queue - Messages Kafka en erreur stockés pour analyse et replay';
COMMENT ON COLUMN dlq_messages.payload_json IS 'Payload du message Kafka en JSON (JSONB pour requêtes)';
COMMENT ON COLUMN dlq_messages.status IS 'Statut: PENDING (en attente), PROCESSING (en cours), RESOLVED (traité), IGNORED (ignoré), FAILED (échec replay)';
